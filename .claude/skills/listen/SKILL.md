...
- **`__LISTEN_FULL_BEGIN__` … `__LISTEN_FULL_END__`** plus
  ...
  **ANTI-FABRICATION RULE (this is the one you keep breaking — obey it):**
  ...
  Format:
  1. Headline the captured latest question + its KB-cited answer:
     > **⏩ Latest captured question — "<verbatim from __LISTEN_LATEST_QUESTION__>"**
     > **Say:** <ready-to-speak, KB-grounded answer>
  2. Then a brief supporting synthesis from FULL only:
  ...
  Persist it with one `... answer "<markdown>"` call.
...
- `/listen catchup` → answers the remote party's MOST RECENT question first, then a
  brief supporting synthesis. Run this whenever the user just got asked something
  and needs the answer — or asks about the call in plain English.
